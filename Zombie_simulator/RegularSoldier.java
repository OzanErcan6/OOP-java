/**
 * I decleare the String "soldier" for every soldier object and "zombie" for
 * every zombie
 * because at the beginning i put soldiers and zombies into separate lists.
 * other than that soldiers and zombies constructors and functions similar with
 * each other
 * Also i can change collisionRange , starting state etc from constructor easily
 * To improve the code i could add bulletSpeed variable to the constructor
 * instead of when creating bullet typing 100 for sniper for example.
 *
 */
public class RegularSoldier extends SimulationObject {
    ...
    public void step(SimulationController controller) {
         ...
         * – If the position is not out of bounds, change soldier position to the
         * new_position.
         */
        if (this.state == SoldierState.SEARCHING) {
            Position new_position = new Position(
                    this.getPosition().getX() + this.getDirection().getX() * this.getSpeed(),
                    this.getPosition().getY() + this.getDirection().getY() * this.getSpeed());
             ...
             * – If the distance is shorter than or equal to the shooting range of the
             * soldier, change state to AIMING.
             */
         ...
         * – Calculate the euclidean distance (3.1.2) to the closest zombie.
         * – If the distance is shorter than or equal to the shooting range of the
         * soldier, change soldier
         * direction to zombie and change state to SHOOTING. Do not forget to normalize
         * the direction.
        ...
        if (this.state == SoldierState.AIMING) {
            ...
            if (closeZombie != null) {
                distance = this.getPosition().distance(closeZombie.getPosition());
                Position x = new Position(closeZombie.getPosition().getX() - this.getPosition().getX(),
                        closeZombie.getPosition().getY() - this.getPosition().getY());
                x.normalize();
        ...
        /*
         * – Create a bullet. As mentioned before, bullet’s position and direction
         * should be same as sol-
         * dier’s. Speed depends on the soldier which for RegularSoldier is 40.0. Add
         * the bullet to the
         * simulation after all step function are executed.
         * – Calculate the euclidean distance (3.1.2) to the closest zombie.
         * – If the distance is shorter than or equal to the shooting range of the
         * soldier, change state to
         * AIMING.
        ...
        if (this.state == SoldierState.SHOOTING) {
            ...
            SimulationObject newBullet = new Bullet("Bullet" + bulletNo.toString(), this.getPosition(), 40,
                    this.getDirection());
            controller.getBullets().add(newBullet);
            System.out.println(this.getName() + " fired " + newBullet.getName() + " to direction "
                    + newBullet.getDirection().toString());
